...
import { deleteVehicle, getVehicles, updateVehicle } from "@/lib/data";
...
export async function GET(
  ...
  try {
    ...
    const vehicle = (await getVehicles()).find((entry) => entry.id === id);
    if (!vehicle) {
...
export async function PUT(
  ...
  try {
    ...
    const vehicle = (await getVehicles()).find((entry) => entry.id === id) ?? null;
    return apiJson({ vehicle });
  } catch (error) {
    return handleApiError(error);
  }
